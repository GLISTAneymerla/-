...
from keys import draw_keys, create_key_rect
...
key_rect = create_key_rect(len(KEYS))
keys_list = font.SysFont("Arial", 24)
pressed_keys = set()
def start_game(): pass
def open_settings(): pass
def close_settings(): quit()
...
while running:
    ...
    draw_keys(screen, keys_list, pressed_keys)
    display.flip()
    for e in event.get():
        if e.type == QUIT:
            running = False
